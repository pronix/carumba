...
';
	...
	{
		$orders = Array();
		$query = "SELECT SQL_CALC_FOUND_ROWS IF('".date("Y-m-d H:i:00")."' = deliveryDate, 1, 0) as wait, pm_order.orderID,userID, stDate, deliveryDate, prePayment, comment, cardStID, stID, count(pm_order_message.message) as numOfMes, pda 
		FROM pm_order 
	...
	{
		$orders = Array();
		$query = "SELECT pm_order.orderID, userID, stDate, deliveryDate, prePayment, cardStID, stID, comment, count(pm_order_message.message) as numOfMes, pda FROM pm_order
		LEFT JOIN pm_order_message ON pm_order_message.orderID = pm_order.orderID
