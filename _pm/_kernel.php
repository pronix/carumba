...
//    initNotifications(); //$ntfMgr will be created
    ...
    unset($_POST['y']);
    //print_r($_POST);
    $sID = (isset($_POST['comment']['sID'])&&is_numeric($_POST['comment']['sID'])) ? $_POST['comment']['sID'] : 0;
    if ($sID) {
        if (!empty($_POST['comment']['Name']) && !empty($_POST['comment']['Text']) && !empty($_POST['comment']['Code'])) {
            $txtName = mysql_escape_string($_POST['comment']['Name']);
            $txtEmail = mysql_escape_string($_POST['comment']['Email']);
            $txtMessage = mysql_escape_string($_POST['comment']['Text']);
            $oResult = mysql_query("SELECT id FROM pm_comments_codes WHERE code = '{$_POST['comment']['Code']}' LIMIT 1");
            if ($oResult && mysql_num_rows($oResult)) {
                list($id) = mysql_fetch_array($oResult);
                mysql_query("DELETE FROM pm_comments_codes WHERE id = '$id' LIMIT 1");
            } else {
                $sError = 'Введенный код неверен';
            }
        } else {
            $sError = '(*) - Нужно заполнить все обязательные поля';
        }
        
        // Проверить правильность введенного емайла
        if ($id && $txtEmail && !preg_match('/([a-zA-Z0-9_-]+@([a-zA-Z0-9_-]+\.)+[a-zA-Z0-9_-]+)/', $txtEmail)) {
            $sError = 'Введенный Email некорректен';
        }
        
        // Добавить сообщение в базу
        if ($id && !$sError) {
            mysql_query("INSERT INTO pm_comments (cID,sID,date,name,email,comment,public) 
                            VALUES ('','$sID','".time()."','$txtName', '$txtEmail', '$txtMessage', '0')");
            if (mysql_insert_id()) {
                $add = 1;        
                $sError = 'Запись добавлена и будет опубликована после просмотра модератором.';
                mysql_query("DELETE FROM pm_comments_codes WHERE id = '$id' LIMIT 1");
                $_POST['comment']['Name'] = '';
                $_POST['comment']['Email'] = '';
                $_POST['comment']['Code'] = '';
...
function processStdRequest($url)
{
    ...
    // Данные для мета-тэгов страницы
    $md = $structureMgr->getMetaData($pageID);
    
    //die( ($pageID && $md['isHidden']) ? 'yes' : 'no' );
...
function processAdminRequest($url)
{
    ...
    if ($userID == 1 || ($userGroupID != 5 && $userGroupID != 7) ) {
        header('Status: 302 Moved');
        header('Location: /login'); 
        exit();
    }
    ...
    if ($userGroupID == 7)    
    {
        ...
        do
        {
            $i++;
            $pageID1 = $pagePar;
            $pagePar = $structureMgr->getParentPageID($pageID1);
        } while (($pagePar != 1) & ($pagePar != "") & ($i < 10));
        
        if ($pageID1 != 3)
        {
            header('Status: 302 Moved');
            header('Location: /admin?pageID=3'); 
            exit();            
        } 
    }
...
function processOrdersRequest($url)
{
    ...
    //header('Location: ./carorders?cmd=orders&x=5&status=1');    
