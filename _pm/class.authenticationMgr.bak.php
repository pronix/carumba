...
//                trigger_error($sessionID, PM_WARNING);
                ...
                if ($sessionID)
                {
                    $q = "SELECT u.userID FROM pm_sessions s, pm_users u WHERE sessionID=\"" . 
                          mysql_escape_string($sessionID) . "\" AND s.userID=u.userID";
...
//                        trigger_error("Count: $count", PM_WARNING);
        ...
        function getUserData($userID, $login = '', $email = '')
        {
            ...
            if ($login == ""){
                $q = "SELECT * FROM pm_users LEFT JOIN pm_as_cars ON pm_as_cars.carID = pm_users.carID LEFT JOIN pm_as_autocreators ON pm_as_autocreators.plantID = pm_as_cars.plantID WHERE userID=$userID";
            }
            elseif($email) {
				$q = "SELECT * FROM pm_users LEFT JOIN pm_as_cars ON pm_as_cars.carID = pm_users.carID LEFT JOIN pm_as_autocreators ON pm_as_autocreators.plantID = pm_as_cars.plantID WHERE Email='$email'";
            }
			else {
                $q = "SELECT * FROM pm_users LEFT JOIN pm_as_cars ON pm_as_cars.carID = pm_users.carID LEFT JOIN pm_as_autocreators ON pm_as_autocreators.plantID = pm_as_cars.plantID WHERE login='$login'";
			}
			//echo $q.'<br>';
            $qr = mysql_query($q);
