<?php
	...
	class Search extends AbstractModule
	{
		...
		function getContent($args)
		{
			...
			$items = $this->getItems($searchtext, $startFrom, $endAt, $searchtype, $order);
            $cnt = $this->itemsCount;
				...
				else
				{
					if ($pagesCount > 1)
					{
						...
						switch($order) {
							...
							case "order" :$tpl = str_replace("%sel4%", 'selected="selected"', $tpl); break;
						}
						...
						$tpl =str_replace("%catFilter%", "<input type=\"hidden\" name=\"searchtext\" value=\"".$searchtext."\" />", $tpl);
		...
		function getItems($searchtext, $startFrom, $endAt, $searchType = 0, $order)
		{
			//echo $searchtext.'<br>';
			...
			$orderStr = ' ORDER BY ';
			if ($order == 'name') {
				$orderStr .= 's.ShortTitle';
			} elseif ($order == 'price') {
				$orderStr .= 'salePrice';
			} elseif ($order == 'pricedesc') {
				$orderStr .= 'salePrice desc';
			} elseif ($order == 'rating') {
				$orderStr .= 'rating DESC, s.ShortTitle';
			} else {
				$orderStr .= 's.ShortTitle';
			}
			...
			$query = "SELECT SQL_CALC_FOUND_ROWS
								...
								".$orderStr." LIMIT ".$startFrom.",".GetCfg("Search.itemsPerPage")." ";
