 ...
 *
 */
	class New1 extends AbstractModule
	{
		var $itemsCount = 0;
		...
		function getContent($args)
		{
			...
			SetCfg('New1.itemsPerPage', 10);
			SetCfg('New1.itemsPerCol', 1);
			...
			$perPage = GetCfg('New1.itemsPerPage');
			...
			foreach ( $items as $item ) {
				if(($i-1) % GetCfg('New1.itemsPerCol') == 0) {
					$content .= '<tr>';
				}
				$style = 'td'.(($i > GetCfg('New1.itemsPerCol')) ? 'dwn' : 'up').'left';
				$content .= $this->getFilledTemplate($item, $this->HI_DETAILED, $style);
				if($i % GetCfg('New1.itemsPerCol') == 0) {
					$content .= '</tr>';
				}
				$i++;
			}
		...
		function getBlock()
		{
			...
			SetCfg('New1.itemsPerPage', 2);
			SetCfg('New1.itemsPerCol', 2);
			...
			foreach($items as $item) {
				...
				$style = 'td'.(($i > GetCfg('New1.itemsPerCol')) ? 'dwn' : 'up').
				            (($i % GetCfg('New1.itemsPerCol') != 0) ? 'left' : 'right');
			...
			return $content.'<div class="more"><img src="/images/arr_gray2.gif" width="7" height="9"  alt="" /><a href="/main/newproducts/">Подробнее о новинках</a></div></div>';
		}

		function getTopBlock()
		{

			SetCfg('New1.itemsPerPage', 4);
			SetCfg('New1.itemsPerCol', 1);
			...
			return $content.'<div class="more"><img src="/images/arr_gray2.gif" width="7" height="9"  alt="" /><a href="/main/newproducts/">Подробнее о новинках</a></div>';
		}
		...
		function getFilledSmallTemplate($catItem, $detailed = 0)
		{
            ...
            $tpl = $templatesMgr->getTemplate(-1, GetCfg('TemplatesPath') . '/Catalogue/new1.html');
			...
			switch($catItem['ptID']) {
				case '5':
				    $imgPercent = '<img src="/images/new1/sale_b_01.gif" width="70" height="70"  alt="" />';
				    break;
				case '6':
				    $imgPercent = '<img src="/images/new1/sale_b_02.gif" width="70" height="70"  alt="" />';
				    break;
				case '7':
				    $imgPercent = '<img src="/images/new1/sale_b_03.gif" width="70" height="70"  alt="" />';
				    break;
			}

			$tpl = str_replace('%new1%', $imgPercent, $tpl);
		...
		function getRandomBlockItems()
		{
			$query = 'SELECT DISTINCT
					...
					WHERE new=1
					ORDER BY RAND()
					LIMIT '.GetCfg('New1.itemsPerPage');
		...
		function getItems($startFrom, $endAt, $order)
		{
			...
			$query = 'SELECT SQL_CALC_FOUND_ROWS
					accID, p.sID, ShortTitle, deliveryCode, accPlantName, logotype, smallPicture, p.tplID, salePrice,
					MustUseCompatibility, PicturePath, DescriptionTemplate, ptPercent, p.ptID,
					    (SELECT SUM( r.grade ) / r.count /3
                        FROM pm_rating r
                        WHERE r.sID = s.sID
                        ) AS rating
					...
					WHERE new=1 
					'.$orderStr.'
					LIMIT '.$startFrom.','.GetCfg('New1.itemsPerPage');
			$result = mysql_query($query);
			if (!$result)
                trigger_error('Invaid query. ' . mysql_error(), PM_FATAL);
