...
//            $this->desc = "Adds goods from Catalogue to cart";
		...
		function buyCart()
        {
			...
			$userComment = _post("comment_");
            
            //trigger_error($userComment, PM_FATAL);
			...
			if(!$msg) {
				...
				if(mysql_num_rows($result)) {
					...
					$this->sendNotifyEmail($orderID, $userData['userID'], $userEmail, $useCard);
				}
				///////////////////////////////////////////
				header("Status: 302 Moved");
		...
		function sendNotifyEmail($orderID, $userID, $userEmail, $useCard)
		{
        ...
        $mail->Password = "Vifi2Ht6b"; // SMTP password
			...
			$bonusBody = $this->getOrderList($orderID, $useCard);
		...
		function getOrderList($orderID, $useCard)
		{
			global $structureMgr;
            
            //trigger_error( $useCard, PM_FATAL);  
			...
			foreach($items as $item) {
				...
				/*if ($item["ptPercent"] == 0)
                    ...
                    {
                        $firstPrice = $item["salePrice"];
                    }
					//
				else
					$firstPrice = $item["salePrice"] - round($item["salePrice"] * $item["ptPercent"] / 100);
				*/
				...
				if($item["ptPercent"]) {
					$content .= "<p><a href='".$URL."'><img src='http://carumba.ru".$item["smallPicture"]."' width=70 height=70 border=0 /></a><br /><a  href=\"".$URL."\">".$item["ShortTitle"]."</a><br>Цена: <strong>". $firstPrice."</strong> / ".($item["salePrice"]*$item["accCount"])." руб. (- <strong>".$item["ptPercent"]."</strong>%)";
				} else {
					$content .= "<p><a href='".$URL."'><img src='http://carumba.ru".$item["smallPicture"]."' width=70 height=70 border=0 /></a><br /><a  href=\"".$URL."\">".$item["ShortTitle"]."</a><br>Цена: <strong>". $firstPrice."</strong> руб. ";
				}
				$content .= "<br/>Количество: ".$item["accCount"]."</p>";
			}
			$content .= "<p><strong>Итого: ".$sum." руб.</strong></p>";
			$content .= "<p>&nbsp;</p>";
		...
		function getOrderItems($orderID)
		{
			$query = "SELECT 
					...
					MustUseCompatibility, PicturePath, DescriptionTemplate, ptPercent, pm_as_parts.accPlantID, pm_order_parts.accCount, pm_order_parts.price   
					FROM pm_order_parts
