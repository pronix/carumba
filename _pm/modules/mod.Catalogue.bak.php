...
//!!!SD_EDIT                    $sData["tplID"] = array("Шаблон (1 - для запчастей, 2 - для остальных товаров)", "text", 4, $r["tplID"]);
        ...
        function updateSpecificData($args)
        {
            ...
            else
            {
                ...
                switch ($md["DataType"])
                {
                    ...
                    {
						//echo '<br />';
						//print_r($this->dNames[$md["DataType"]]);
						//echo '<br />';
                        foreach ($this->dNames[$md["DataType"]] as $dn)
                        {
                            $v = prepareVar(_post($dn));
		...
		function getCategoryFilter($pageID, &$branch)
        {
            ...
            //We need to generate cars list
            {
                ...
                $carlist .= "<input type=\"radio\" name=\"carID\" value=\"0\" checked=\"checked\" onclick=\"showCatForCar('catForCar0', '".count($cars)."');\" /> <a class=\"levm\" href=\"?carID=0\">все</a><br />\n";
                ...
                {
                    $URL = "?carID=$carID";
                    $carlist .= "<input type=\"radio\" name=\"carID\" value=\"$carID\" onclick=\"showCatForCar('catForCar$carID', '".count($cars)."');\" /> <a class=\"levm\" href=\"$URL\">" . $car[0] . " " . $car[1] . "</a><br/>\n";
...
//////////////////Важное место////////////////////
						...
						if($this->FILTER_CONSTANT_ARRAY[$pageID]['showSubCats']) {
							$res .= ($i > 0 ? "<br />":"").$this->addInputType("c-$i", $bsID, "<a  href=\"?c-$i=$bsID\" class=\"bllink\"><strong>" . $branch[$i]["Title"] . " ($chCount)</strong></a>", $this->FILTER_CONSTANT_ARRAY[$pageID]['catType']).(count($subCats)?": ":"")."<br />\n";
...
//                        $res .= "<input type=\"checkbox\"> <a class=\"levm\" href=\"$URL\">" . $branch[$i]["Title"] . " ($chCount)</a><br/>\n";
		...
		function getItemsWithFilter($props, $startFrom, $endAt, $from, $to, $propID, $carID, $order) 
		{
			...
			if($propID) {
				...
				$joinStr .= " LEFT JOIN pm_as_parts_properties pp1 ON (pp1.accID = p.accID && pp1.propListID = '2') ";
			}
			if($carID) {
        ...
        function getFilledItemDescriptionTemplate($catItem)
        {
			...
			$htmlRating = '<span class="red" id="ratingStat">'.$fRating.'</span>&nbsp;
			          - Рейтинг <select class="ratinglist" name="rating" id="ratingValue">
                    ...
                    </select><input src="/images/butt_ok2.gif" type="image" id="ratingSend" />
                    <input type="hidden" name="ratingsID" id="ratingsID" value="'.$sID.'" />
                    <div id="ratingStatus" style="display:none; text-align:center;"></div>';
        ...
        function getCatItemByPageID($pageID)
        {
            ...
            $q = "
                 SELECT accID, p.sID, ShortTitle, deliveryCode, accPlantName, logotype, smallPicture, s.tplID, salePrice, 
                 MustUseCompatibility, PicturePath, DescriptionTemplate, ptPercent, p.ptID  
                 FROM `pm_as_parts` p
                 LEFT JOIN pm_as_producer ap ON (ap.accPlantID = p.accPlantID)
                 LEFT JOIN pm_structure s ON (p.sID = s.sID)
