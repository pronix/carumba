...
';
	...
	{
		$statuses = Array();
		$query = "SELECT COUNT(pm_order.stID) as num, pm_order_status.stID, pm_order_status.name FROM pm_order_status LEFT JOIN pm_order ON pm_order.stID = pm_order_status.stID GROUP BY pm_order_status.stID";
		$result = mysql_query($query);
		if(mysql_num_rows($result)) {
