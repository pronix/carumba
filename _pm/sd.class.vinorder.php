...
';
	...
	{
		$statuses = Array();
		$query = "SELECT COUNT(pm_vinorder.stID) as num, pm_vinorder_status.stID, pm_vinorder_status.name FROM pm_vinorder_status LEFT JOIN pm_vinorder ON pm_vinorder.stID = pm_vinorder_status.stID AND orderType=0 GROUP BY pm_vinorder_status.stID";
		$result = mysql_query($query);
		if(mysql_num_rows($result)) {
			while($row = mysql_fetch_assoc($result)) {
				$statuses[] = $row;	
	...
	function getOrders($orderStatus, $start, $count)
	{
		...
		WHERE stID = '".$orderStatus."' AND orderType=0
