...
';
	...
	function sendReply()
	{
		...
		$mail->Subject = "Ответ на закак carumba.ru";
		$mail->Body = $reply;
	...
	function sendPart()
	{
		...
		$body .= "Заказанные Вами запчасти: <br>";
		...
		for($i = 1 ; $i < 4; $i++) {
			...
			//echo $orderStatusQuery.'<br />';
			mysql_query($orderpartsDetails);
			$body .= "Запчасть №".$i."<br>Наименование: "._post("accName_".$i)."<br>"."Код: "._post("accID_".$i)."<br>Цена продажи: "._post("accSalePrice_".$i)."<br>Дата поставки: "._post("day_".$i).'/'._post("month_".$i).'/'._post("year_".$i)."<br>";
		}
		...
		$mail->WordWrap = 50;                                 // set word wrap to 50 characters
		$mail->IsHTML(true);                                  // set email format to HTML
		$mail->Subject = "Изменения по заказу на carumba.ru";
		$mail->Body = $body;
		$mail->AddAddress($userData['Email'], $userData['LastName']." ".$userData['FirstName']);
		if(!$mail->Send())
		{
	...
	function getOrderRow($order, $style = "")
	{
		...
		<td>'.$userID.'<br> </td>
		<td> <a style="cursor: pointer" onClick="openPopupWindow(\'/carorders?cmd=vinorders&act=anket&orderID='.$order['orderID'].'\', 600, 500); return false;">Анкета</a></td>
		<td bgcolor="'.$style['bgcolor'].'"><p> <a style="cursor: pointer" onClick="openPopupWindow(\'/carorders?cmd=vinorders&act=order&orderID='.$order['orderID'].'\', 600, 500); return false">Посмотреть</a><br>
			'.$numDetails.' товара(ов) </p></td>
		<td bgcolor="'.$style['bgcolor'].'">'.$priceDetails.'</td>
		<td> <a style="cursor: pointer" onClick="openPopupWindow(\'/carorders?cmd=vinorders&act=data&orderID='.$order['orderID'].'\', 600, 500); return false">История дат</a> <br>
		  '.$order['stDate'].'</td>
		<td '.($order['wait'] ? 'class="today"' : "" ).'> <a style="cursor: pointer" onClick="openPopupWindow(\'/carorders?cmd=vinorders&act=delivery&orderID='.$order['orderID'].'\', 600, 500); return false">'.($order['deliveryDate'] == "0000-00-00" ? 'Назначить' : 'Изменить').'</a><br>
		  '.$order['deliveryDate'].'</td>
		<td><p> <a style="cursor: pointer" onClick="openPopupWindow(\'/carorders?cmd=vinorders&act=prepayment&orderID='.$order['orderID'].'\', 600, 500); return false">Изменить</a><br>
			'.$order['prePayment'].' руб. </p></td>
		<td><a style="cursor: pointer" onClick="openPopupWindow(\'/carorders?cmd=vinorders&act=send&orderID='.$order['orderID'].'\', 600, 500); return false">Послать</a><br>
			'.($order['numOfMes'] ? '<a style="cursor: pointer" onClick="openPopupWindow(\'/carorders?cmd=vinorders&act=viewReplies&orderID='.$order['orderID'].'\', 600, 500); return false">Читать</a>' : '').'
		</td>
	...
	{
		$statuses = Array();
		$query = "SELECT COUNT(pm_vinorder.stID) as num, pm_vinorder_status.stID, pm_vinorder_status.name FROM pm_vinorder_status LEFT JOIN pm_vinorder ON pm_vinorder.stID = pm_vinorder_status.stID AND orderType=1 GROUP BY pm_vinorder_status.stID";
		$result = mysql_query($query);
		if(mysql_num_rows($result)) {
			while($row = mysql_fetch_assoc($result)) {
				$statuses[] = $row;	
	...
	function getOrders($orderStatus, $start, $count)
	{
		...
		WHERE stID = '".$orderStatus."' AND orderType=1
