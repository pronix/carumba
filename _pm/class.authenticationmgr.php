...
//                        trigger_error("Count: $count", PM_WARNING);
        ...
        function authenticate()
        {
            ...
            $q = "SELECT Login FROM adm_users_ip WHERE login=\"" . mysql_escape_string(_post('login')) . "\"";
            ...
            $cnt_l = mysql_num_rows($qr1);

            if ($cnt_l > 0)            
            {
                
                $q = "SELECT Login FROM adm_users_ip WHERE Login=\"" . mysql_escape_string(_post('login')) . "\" and ip=\"" . $_SERVER["REMOTE_ADDR"] . "\"";
...
//                    trigger_error("Auth userID: $userID", PM_WARNING);
                    $sessionID = (isset($_COOKIE['sessionID'])) ? $_COOKIE['sessionID'] : false;
//                    trigger_error("Auth sessionID: $sessionID", PM_WARNING);
        ...
        {
            return (isset($_COOKIE['sessionID'])) ? $_COOKIE['sessionID'] : false;
        }
