            ...
            if ($dtForCols == $branch[$i]["DataType"])
            {
                switch ($addColVal[1])
                {
                    ...
                    case "text": 
                        ...
                        break;
                    case "dropdown": 
                        $res .= "<script>writeDropDown('$addCol', '" . $branch[$i]["sID"]. "', '" . $dataList[$branch[$i]["sID"]][$addCol]. "');</script>";//$addColVal[2][1];//" - ";
                        break;
                    case "checkbox": 
...
function getOptionBranches($pageID)
{
	global $structureMgr;
	...
	$branch = $structureMgr->getStructureForPageID($pageID, 5);
