...
*/
    class TemplatesManager
    {
        ...
        {
            $q = "SELECT val FROM pm_config WHERE var='DefaultTplID' LIMIT 1";
            $qr = mysql_query($q);
            if (!$qr)
            {
                trigger_error("DefaultTplID not found in configuration table", PM_WARNING);

                $q = "SELECT MIN(tplID) FROM pm_templates LIMIT 1";
                $qr = mysql_query($q);
                if (!$qr)
        ...
        function getTemplate($templateID, $fileName)
        {
            if ($templateID != -1)
            {
                $q = 'SELECT tplFilename FROM pm_templates WHERE tplID = "'.$templateID.'" LIMIT 1';
                $qr = mysql_query($q);
