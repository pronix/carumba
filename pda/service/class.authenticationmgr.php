...
//                        trigger_error("Count: $count", PM_WARNING);
        ...
        function authenticate($login, $psw)
        {
            $q = "SELECT userID FROM pm_users WHERE login=\"" . mysql_escape_string($login) . 
                 "\" AND Password = \""  . mysql_escape_string(md5($psw)) . "\" AND uDeleted=0 AND isUserGroup=0";
